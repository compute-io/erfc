...
*
* @param {Number|Number[]|Array} x - input value over which to evaluate the complementary error function
* @param {Object} [options] - function options
* @param {Function} [options.accessor] - accessor function for accessing array values
* @param {Boolean} [options.copy=true] - boolean indicating whether to return a new array
* @returns {Number|Number[]} evaluated complementary error function
*/
function evaluate( x, opts ) {
	...
	if ( !isArray( x ) ) {
		throw new TypeError( 'erfc()::invalid input argument. Must provide either a single numeric value or an array. Value: `' + x + '`.' );
	}

	if ( arguments.length > 1 ) {
		if ( !isObject( opts ) ) {
			throw new TypeError( 'erfc()::invalid input argument. Options argument must be an object. Value: `' + opts + '`.' );
		}
		if ( opts.hasOwnProperty( 'accessor' ) ) {
			clbk = opts.accessor;
			if ( !isFunction( clbk ) ) {
				throw new TypeError( 'erfc()::invalid option. Accessor must be a function. Option: `' + clbk + '`.' );
			}
		}
		if ( opts.hasOwnProperty( 'copy' ) ) {
			copy = opts.copy;
			if ( !isBoolean( copy ) ) {
				throw new TypeError( 'erfc()::invalid option. Copy option must be a boolean primitive. Option: `' + copy + '`.' );
			}
		}
	}
	len = x.length;
	...
	if ( clbk ) {
		for ( i = 0; i < len; i++ ) {
			v = clbk( x[ i ], i );
			if ( typeof v !== 'number' ) {
				throw new TypeError( 'erfc()::invalid value. Accessed array values must be number primitives. Value: `' + v + '`.' );
			}
			arr[ i ] = erfc( v );
		}
	} else {
		for ( i = 0; i < len; i++ ) {
			v = x[ i ];
			if ( typeof v !== 'number' ) {
				throw new TypeError( 'erfc()::invalid input argument. Input array must only contain number primitives. Value: `' + v + '`.' );
			}
			arr[ i ] = erfc( v );
		}
	}
